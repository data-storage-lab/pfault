...
ssh $ISCSI_SERVER_USER@$ISCSI_SERVER_IP <<EOF
 chmod 777 $ISCSI_DIR/pfe_scripts/multinodes/configuration.sh
 cd $ISCSI_DIR/pfe_scripts/multinodes/
 ./bk_create.sh
 sleep 10
...
do
 ssh ${CLIENT_USER[it]}@${CLIENT_IP[it]} <<EOF
  mount.lustre ${MGS_IP[0]}@${MGS_LNET_POSTFIX[0]}:/${LUSTRE_NAME[0]} ${CLIENT_LUSTRE_MNT_PNT[it]}
  sleep 2
  lfs setstripe -i 0 -c -1 -S 64K ${CLIENT_LUSTRE_MNT_PNT[it]}
  lfs df -h
EOF
 let "it+=1"
done
